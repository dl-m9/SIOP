...
class RayPPOTrainer:
    ...
    def _validate(self, merged: bool = False):
        ...
        from tqdm import tqdm
        val_iter = tqdm(self.val_dataloader, desc="Validation", unit="batch", file=__import__('sys').stdout)
    ...
    def _val_metrics_update(self, data_sources, sample_uids, reward_extra_infos_dict, sample_turns):
        ...
        # Unweighted mean of per-benchmark acc@max_n (each data_source one vote). Complements
        # per-dataset val-core/*/acc/mean@N — those are within-benchmark averages only.
        for var_key in ("acc", "f1"):
            macro_vals = []
            for _ds, var2metric2val in data_src2var2metric2val.items():
                if var_key not in var2metric2val:
                    continue
                metric2val = var2metric2val[var_key]
                n_max = max(int(name.split("@")[-1].split("/")[0]) for name in metric2val.keys())
                key = f"mean@{n_max}"
                if key in metric2val:
                    macro_vals.append(float(metric2val[key]))
            if macro_vals:
                metric_dict[f"val-core/macro/{var_key}/mean"] = float(np.mean(macro_vals))
    ...
    def fit(self):
        ...
        # Also validate on resume if the checkpoint step aligns with test_freq
        # (handles the case where training crashed during/after eval)
        test_freq = self.config.trainer.get("test_freq", -1)
        skip_resume_val = self.config.trainer.get("skip_resume_val", False)
        val_on_resume = (
            self.global_steps > 0
            and test_freq > 0
            and self.global_steps % test_freq == 0
            and not skip_resume_val
        )
        if self.config.trainer.get("val_before_train", True) or val_on_resume:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("gen", timing_raw, color="red"):
                        print(
                            ...
                            f"batch_size={len(gen_batch_output)} "
                            f"rollout_n={self.config.actor_rollout_ref.rollout.n} "
                            f"multi_turn={self.config.actor_rollout_ref.rollout.multi_turn.enable}"
                        )
                    ...
                    # SIOP: cluster + score (via external vLLM server) + process rewards
                    if self.config.algorithm.adv_estimator == "siop":
                        if getattr(self.config.algorithm, "siop_enable_two_pass", True):
                            try:
                                with marked_timer("siop", timing_raw, color="magenta"):
                                    from verl.utils.siop.siop_process_reward import compute_siop_rewards_full
                                    siop_rewards = compute_siop_rewards_full(
                                        batch=batch, tokenizer=self.tokenizer, config=self.config.algorithm,
                                    )
                                    batch.non_tensor_batch["siop_process_rewards"] = siop_rewards
                            except Exception as e:
                                print(f"[SIOP] Error in SIOP computation, falling back to no process rewards: {e}",
                                      flush=True)
                        else:
                            print("[SIOP] siop_enable_two_pass=False, skipping.", flush=True)
