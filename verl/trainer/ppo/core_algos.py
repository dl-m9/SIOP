...
# ============================================================
# IGPO: Turn-level advantage computation
# ============================================================
...
def _compute_turn_level_advantage(
    ...
    1. Identify turn boundaries for each sample
...
@register_adv_est("igpo")
def compute_igpo_turn_level_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    config=None,
    ...
    **kwargs,
    ...
    """IGPO turn-level advantage: separate F1/IG normalization + discounted accumulation."""
    gamma = getattr(config, "gamma", 1.0) if config else 1.0
    norm_mode = getattr(config, "info_gain_norm_mode", "separate") if config else "separate"
    curriculum_f1 = getattr(config, "curriculum_f1_weight", 1.0) if config else 1.0
    curriculum_ig = getattr(config, "curriculum_ig_weight", 1.0) if config else 1.0
    norm_by_std = getattr(config, "norm_adv_by_std_in_grpo", True) if config else True
    ...
    # Step 1.5: Curriculum weights
    if curriculum_f1 != 1.0 or curriculum_ig != 1.0:
        weighted = token_level_rewards.clone()
        weighted = torch.where(f1_mask, token_level_rewards * curriculum_f1, weighted)
        weighted = torch.where(ig_mask, token_level_rewards * curriculum_ig, weighted)
        token_level_rewards = weighted
    ...
    if norm_mode == "separate":
        ...
        norm_f1 = (token_level_rewards - f1_mean[group_ids_expanded])
        if norm_by_std:
            norm_f1 = norm_f1 / (f1_std[group_ids_expanded] + epsilon)
...
# ============================================================
# SIOP: Self-Induced Outcome Potentials advantage computation
# ============================================================

@register_adv_est("siop")
def compute_siop_turn_level_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    config=None,
    ...
    **kwargs,
    ...
    """SIOP turn-level advantage: per-query group z-normalization + discounted accumulation.

    Matches paper's Section "Turn-Level Advantage Construction": for each query group
    (K rollouts), pool all (rollout, turn) reward pairs within that group and compute
    mean/std locally.  This is GRPO-style per-group normalization, not batch-global.
    """
    if config is not None and not getattr(config, "siop_enable_two_pass", True):
    ...
    # Ablation B2: keep SIOP process rewards but collapse to outcome-level advantage
    # (tests whether the turn-level credit assignment itself adds value beyond the reward).
    advantage_mode = getattr(config, "siop_advantage_mode", "turn_level") if config else "turn_level"
    ...
    # Detect turn boundaries from response_mask (1→0 transitions = assistant turn ends).
    # This is robust to zero-valued rewards (e.g., first turn process reward = 0.0).
    # For each sample, the last token of each contiguous run of 1s in response_mask
    # is a turn boundary.
    padded = torch.nn.functional.pad(response_mask, (0, 1), value=0)  # (bsz, seq_len+1)
    turn_end_mask = (padded[:, :-1] == 1) & (padded[:, 1:] == 0)     # (bsz, seq_len)

    # reward_mask: positions where we placed rewards (turn boundaries within response)
    reward_mask = turn_end_mask & (response_mask == 1)

    # Per-query group z-normalization (matches paper):
    # For each query, pool all (rollout, turn) reward pairs within its K rollouts,
    # then normalize with the group's own mean/std.
    normalized_rewards = torch.zeros_like(token_level_rewards)
    unique_uids = np.unique(index)
    for uid in unique_uids:
        group_sample_mask_np = (index == uid)
        group_sample_mask = torch.from_numpy(group_sample_mask_np).to(device)  # (bsz,)
        # Only reward positions within this group's samples
        group_reward_mask = reward_mask & group_sample_mask.unsqueeze(1)

        group_rewards = token_level_rewards[group_reward_mask]
        if group_rewards.numel() <= 1:
            # Can't compute meaningful std → advantages for this group stay 0
            continue

        g_mean = group_rewards.mean()
        g_std = group_rewards.std()
        if g_std < epsilon:
            g_std = torch.tensor(1.0, device=device)

        norm_vals = token_level_rewards - g_mean
        if norm_by_std:
            norm_vals = norm_vals / (g_std + epsilon)
    ...
    # Turn-level discounted accumulation + broadcast (reuse IGPO helper)
    discounted_returns = _compute_turn_level_advantage(
        normalized_rewards=normalized_rewards,
        response_mask=response_mask,
        gamma=gamma,
        ...
        seq_len=seq_len,
        device=device,
        turn_boundary_mask=reward_mask,
    )
