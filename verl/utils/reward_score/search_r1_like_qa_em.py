...
def _extract_question_for_logging(extra_info):
    if not extra_info:
        return "N/A"
...
def compute_score(solution_str, ground_truth, method="strict", format_score=0.0, score=1.0, extra_info=None):
    ...
    if do_print:
        question = _extract_question_for_logging(extra_info)
        print(f"======== [reward-sample] question={question} ========")
