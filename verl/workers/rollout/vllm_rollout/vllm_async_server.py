...
class vLLMHttpServer:
    ...
    async def _maybe_get_lora_request(self) -> Optional[LoRARequest]:
    ...
    async def generate(
        ...
        # Add lora request
        lora_request = await self._maybe_get_lora_request()
    ...
    async def score_prompts(
        ...
        request_id_prefix: str = "siop_score",
        priority: int = 0,
    ) -> list[float]:
        """Score prompt token ranges with prompt_logprobs on the existing rollout engine."""

        if len(prompt_ids_list) != len(score_ranges):
            raise ValueError(
                f"prompt_ids_list and score_ranges length mismatch: {len(prompt_ids_list)} vs {len(score_ranges)}"
            )
        ...
        async def _score_one(sample_idx: int, prompt_ids: list[int], score_range: tuple[int, int]) -> float:
            prompt_ids = normalize_token_ids(prompt_ids)
            start_idx, end_idx = score_range

            if start_idx < 0 or end_idx < start_idx or end_idx > len(prompt_ids):
                raise ValueError(
                    f"Invalid score_range {score_range} for prompt length {len(prompt_ids)} at sample_idx={sample_idx}"
                )

            # vLLM 0.11.0 requires max_tokens >= 1, so we request one extra token
            # and ignore the generated output, using prompt_logprobs only.
            if len(prompt_ids) >= self.config.max_model_len:
                raise ValueError(
                    f"Prompt length ({len(prompt_ids)}) must be smaller than max_model_len "
                    f"({self.config.max_model_len}) for vLLM prompt_logprobs scoring."
                )
            ...
            generator = self.engine.generate(
                prompt=prompt,
                sampling_params=sampling_params,
                request_id=f"{request_id_prefix}_{sample_idx}_{uuid4().hex}",
                lora_request=lora_request,
                priority=priority,
            )
            ...
            if prompt_logprobs is None:
                raise RuntimeError("vLLM returned no prompt_logprobs for score_prompts request.")
            ...
            for pos in range(start_idx, end_idx):
                ...
                if position_logprobs is None or token_id not in position_logprobs:
                    raise RuntimeError(
                        f"Missing prompt logprob for token position {pos} (token_id={token_id}) in sample_idx={sample_idx}"
                    )
                token_logprobs.append(position_logprobs[token_id].logprob)
        ...
        tasks = [
            _score_one(sample_idx=i, prompt_ids=prompt_ids, score_range=score_range)
            for i, (prompt_ids, score_range) in enumerate(zip(prompt_ids_list, score_ranges, strict=True))
        ]
